...
ns = api.namespace ('gan_client', description='Operation for GAN client')

# Flask-RestPlus specific parser for image uploading
UPLOAD_KEY = 'image'
UPLOAD_LOCATTION = 'files'
upload_parser = api.parser ()
upload_parser.add_argument (UPLOAD_KEY,
                            location=UPLOAD_LOCATTION,
                            type=FileStorage,
                            required=True)
...
class GanPrediction (Resource):
    @ns.doc (description='Predict the house number on the image using GAN model.' +
                         'Return 3 most problable digits with their probabilities ',
             responses={
                 200: "Success",
                 400: "Bad request",
    ...
    def post(self):
        try:
            ...
            return {'message': 'something wrong with the incoming request . ' +
                               'Original message : {}'.format (inst)}, 400

        try:
            results = make_prediction (image.read ())
            results_json = [{'digits': res[0], 'probability': res[1]} for res in results]
